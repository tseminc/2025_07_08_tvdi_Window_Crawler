...
class SimpleApp:
    ...
    def create_widgets(self):
        ...
        # 手動插入股票資料
        #print(f'股票資料共 : {len(self.stock_codes)} 筆')
        ...
        # self.selected_label.config(text=f"選取的股票數量是:  0筆")
        # self.selected_label.pack(pady=10)
        self.selected_button = tk.Button(root_right_frame, text=f'選取的股票數量是:{0:3}筆', font=("Arial"),state=tk.DISABLED)
    ...
    def on_stock_select(self, event):
        ...
        #self.selected_label.config(text=f"選取的股票數量是:{len(self.selected_stocks) : 3}筆")
    ...
    def clear_selection(self):
        ...
        self.selected_button.config(text=f'選取的股票數量是:{0:3}筆')
        self.selected_button.config(state=tk.DISABLED)      
        #self.selected_label.config(text=f"選取的股票數量是:  0筆")
